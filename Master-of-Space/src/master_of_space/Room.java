...
/**
 * Created by wassing on 2016-04-04. Room keep track of everything currently happening on screen or in the currecnt "room". Also
 * keep the Player, stats, game over etc.
 */
...
public class Room
{
    ...
    @SuppressWarnings("SuspiciousGetterSetter") public int getPixelWidthPerTile() {
	return PIXELWIDTH_PER_TILE;
    } //used by painter

    @SuppressWarnings("SuspiciousGetterSetter") public int getPixelHeightPerTile() {
	...
	while (e.hasNext()) {
		...
		try {
		    e.remove();
		} catch (IllegalStateException ignored) {}        // Might occur if two collisions occur at the same time. Everything will solve itself next tick so we don't need to do anything
	    } else {
		enemy.move();
    ...
    /**
     * This function is used by the paint component to find all objects on each specific square on the board. It first checks
     * the player, then enemies, shots and other objects
     */
